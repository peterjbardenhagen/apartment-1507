...
<template>
  <div class="space-y-6 animate-fade-in">
    <div>
      <h1 class="text-2xl font-bold text-slate-900">Guest Tracking</h1>
      <p class="text-sm text-slate-500 mt-1">Monitor guest nights and calculate extra charges.</p>
    </div>

    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
      <div class="card p-5">
        <p class="text-sm text-slate-600">Guests staying more than 2 nights/week incur extra charges.</p>
        <p class="text-sm text-slate-500 mt-1">Rate: ${{ tenantStore.tenants[0]?.rent || 450 }}/week ÷ 7 nights = ${{ ((tenantStore.tenants[0]?.rent || 450) / 7).toFixed(2) }}/night</p>
      </div>
      <div class="card p-5">
        <p class="text-sm text-slate-600">Free nights per week:</p>
      ...
      <table class="w-full">
        ...
        <tbody>
          <tr v-for="guest in allGuests" :key="guest.id" class="border-b border-slate-100 hover:bg-slate-50 transition">
            <td class="p-4 text-slate-900 font-medium">{{ guest.name }}</td>
            <td class="p-4 text-slate-600">{{ guest.tenantName }}</td>
            <td class="p-4 text-slate-900">{{ guest.nights }}</td>
